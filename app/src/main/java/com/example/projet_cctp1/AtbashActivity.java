...
public class AtbashActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    //crypter
    public static String AtbashCodage(String clair) {
        String codé = "";
        int ctoascii = 0;
        for(int i = 0; i<clair.length(); i++) {
            ...
            int asciiTransf = AtbashAscii(ctoascii);
            //asciiToChar
            //System.out.print(asciiTransf + " ");

            //2 cas d'encodage, si le char est dit affichable ou bien si il ne l'est pas:
            if(!(asciiTransf < 32 || asciiTransf == 127 || asciiTransf == 255)) {
                char asciiToChar = AsciiToChar(asciiTransf); // j'utilise la fonction AsciiToChar
                //rajouter au mot codé
                codé += asciiToChar;
               // System.out.println(" char : " + asciiToChar);
            } else {
                if(asciiTransf < 16) {
                    String hex = Integer.toHexString(asciiTransf);
    ...
    public static String AtbashDécodage(String codé) {
        String clair = "";
        int ctoascii = 0;
        for( int i = 0; i < codé.length(); i++) {
            ...
            int asciiDecodage = AtbashAscii(ctoascii); // applique l'algorithme de décodage sur l'ascii pour otenir l'ascii
    ...
    public static int AtbashAscii(int ascii) {
        int début = 0;
        int fin = 255;
        int asciiobtenu = (fin - ascii) + début;
        return asciiobtenu;
    }

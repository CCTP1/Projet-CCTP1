...
public class VigenereActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        ...
        if(v.getId() == R.id.décrypter){
            ...
            clé = textclé.getText().toString();
            clair = VigenèreDécodage(crypte, clé);
    ...
    public static String Vigenère(String clair, String clé) {
        String codé = "";
        int clélength = clé.length();
        int ctoascii = 0;
        int iclé = 0;
        int clétoascii = 0;
        for(int i = 0; i<clair.length();i++) {
            // récupère chaque caractère un à un du message clair et de la clé
            char caractère = clair.charAt(i);
            //System.out.println(iclé);
            char CharClé = clé.charAt(iclé%clélength);

            //System.out.println(caractère + " "+ CharClé);
            // converti en code Ascii chaque caractère du mot
            ...
            // converti en code ascii chaque caractère de la clé
            if (clé.length() - iclé >= 4 && clé.charAt(iclé) == '\\' && clé.charAt(iclé+1) == 'x' &&
                    (clé.charAt(iclé+2) >= 'a' && clé.charAt(iclé+2) <= 'f' ||
                            clé.charAt(iclé+2) >= '0' && clé.charAt(iclé+2) <= '9')
                    && (clé.charAt(iclé+3) >= 'a' && clé.charAt(iclé+3) <= 'f' ||
                    clé.charAt(iclé+3) >= '0' && clé.charAt(iclé+3) <= '9')) /*si on récupère un hexa*/{
                //System.out.println(i +" : "+ clair.charAt(i) +" "+ (i+1) +" : "+ clair.charAt(i+1) +" "+ (i+2) +" : "+ codé.charAt(i+2)  +" "+ (i+3) +" : "+ codé.charAt(i+3));
                String hex = "" + clé.charAt(iclé+2) + clé.charAt(iclé+3);
                int asciiHex = Integer.parseInt(hex, 16);
                //System.out.println(asciiHex);
                clétoascii = asciiHex; //assignation de la valeur de l'ascii du caractère à décodé à ctoascii
                iclé += 3;
            }else { // cas où l'on récupère un caractère affichable
                clétoascii = charToAscii(CharClé); // converti le caractère en code ascii;
            }

            //System.out.println(ctoascii + " "+ clétoascii);
            //opération cryptographique
            int asciiCodage = VigenèreAscii(ctoascii, clétoascii);
            //asciiToChar
            ...
            iclé ++;
            if (iclé >= clé.length()) {
                iclé = iclé%clé.length();
            }
        }
        return codé;
    }

    public static String VigenèreDécodage(String clair, String clé) {
        String codé = "";
        int clélength = clé.length();
        int ctoascii = 0;
        int iclé = 0;
        ...
        for(int i = 0; i<clair.length();i++) {
            ...
            //System.out.println(ctoascii + " "+ clétoascii);
            //opération cryptographique
            int asciiCodage = VigenèreAsciiDécodage(ctoascii, clétoascii);
            //asciiToChar
            if(!(asciiCodage < 32 || asciiCodage == 127 || asciiCodage == 255)) {
                char asciiToChar = AsciiToChar(asciiCodage); // j'utilise la fonction AsciiToChar
                //rajouter au mot codé
    ...
    public static int VigenèreAsciiDécodage(int ascii, int clé) {
        //int début = (int)'a';
        //int fin = (int)'z';
    ...
    public static int VigenèreAscii(int ascii, int clé) {
        //int début = (int)'a';
        //int fin = (int)'z';
    ...
    public static int charToAscii(char caractère) {
        ...
        if(codecaractère > 127) { // le int renvoyé par le caractère est > 127 c'est un ascii étendu
            ...
            //tous les codes ascii étendues sont affichable sauf le 255 ieme caractère du coup ici on est pas obligé de faire un code
